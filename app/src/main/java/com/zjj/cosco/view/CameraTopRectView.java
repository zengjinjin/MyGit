...
public class CameraTopRectView extends View {
    ...
    public CameraTopRectView(Context context, AttributeSet attrs) {
        ...
        panelWidth = GuideUtil.getInstance().getScreenWidth(activity);
        panelHeght = GuideUtil.getInstance().getScreenHeight(activity);
        Log.i("zjj","panelWidth="+panelWidth+",panelHeght="+panelHeght);

        viewHeight = panelHeght - GuideUtil.getInstance().dip2px(activity, TOP_BAR_HEIGHT + BOTTOM_BTN_HEIGHT);
        viewWidth = panelWidth;

        rectWidth = panelWidth - GuideUtil.getInstance().dip2px(activity, LEFT_PADDING + RIGHT_PADDING);
